...
export function textColorShade(startColor: string, endColor: string) {
  return chroma
    .scale([startColor, "gray", endColor])
    .mode("lab")
...
export const getTextColor = (color?: string) => {
  ...
  const alpha = chroma(color).rgba()[3];
  console.log("getTextColor -> alpha", alpha);
  if (!alpha || alpha > 0.5) {
